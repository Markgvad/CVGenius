...
exports.getSubscriptionStatus = async (req, res) => {
  try {
    ...
    res.json({
      ...
      allowedCVs: user.subscription?.features?.allowedCVs || 1,
      hasAnalytics: user.subscription?.features?.hasAnalytics || false
    });
  } catch (error) {
...
exports.verifySession = async (req, res) => {
  try {
    ...
    if (user.subscription?.tier !== session.metadata.plan) {
      logger.info(`Updating user subscription from ${user.subscription?.tier} to ${session.metadata.plan}`);
